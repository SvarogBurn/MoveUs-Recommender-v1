...
def validate_location(
    ...
    if location_address_line1 and len(location_address_line1) > 255:
        raise MUError(MUErrorCode.ADDRESS_LINE_MAX_LENGTH)

    if location_address_line2 and len(location_address_line2) > 255:
        raise MUError(MUErrorCode.ADDRESS_LINE_MAX_LENGTH)

    if location_zip_code and not 1e5 <= location_zip_code < 1e6:
        raise MUError(MUErrorCode.ZIP_CODE_VALUE)

    if location_region and len(location_region) > 128:
