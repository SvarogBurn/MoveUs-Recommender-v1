...
for directory in subdirectories:
    try:
        module = importlib.import_module(f"api.graphql.{directory}.subscriptions")
        if module:
...
for base_class in subscriptions_base_classes:
    try:
        properties.update(base_class._meta.fields)
    except (AttributeError, KeyError):
        pass

Subscriptions = type("Subscriptions", tuple(subscriptions_base_classes), properties)
